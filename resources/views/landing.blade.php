...
<?php
  ...
  $questions = array("What method of dying do you fear the most?",
                     "What is your opinion on the current state of academics?",
...
<script>
  ...
  function validate_q() {
    ...
    if(str.length < Q_MIN_LENGTH || str.length > Q_MAX_LENGTH || str.charAt(str.length-1) != "?") {
      var errmsg;
      if(str.length < Q_MIN_LENGTH) errmsg = "The question must be at least "+Q_MIN_LENGTH+" characters.";
      else if(str.length > Q_MAX_LENGTH) errmsg = "The question must be no longer than "+Q_MAX_LENGTH+" characters.";
      else if(str.charAt(str.length-1) != "?") errmsg = "The question must end with a question mark (?).";
...
<body>
  ...
  <div class="jumbotron text-center"> 
    ...
    <h3> <small> Ask questions, get answers </small> </h3>
  </div>

  <h3 class="container panel well"><a href="#question" data-toggle="collapse"> Ask a question <span class="glyphicon glyphicon-question-sign"></span></a></h3>

  <div id="question" class="container text-center form-group collapse">
    <form class="" id="q_form" method="post" action="question"> @csrf
  ...
  <h3 class="container well"><a href="#answers" data-toggle="collapse"> Answer a question <span class="glyphicon glyphicon-pencil"></span> </a></h3>
  <div id="answers" class="collapse">
    <ul class="list-group container">
      <?php
        $questions = DB::table('question')->orderBy('id', 'DESC')->get();
        foreach($questions as $question) {
          echo "<a class=\"list-group-item\" href=\"question/$question->id\">
                  $question->text";
